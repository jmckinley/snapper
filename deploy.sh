...
#   --splunk          Configure Splunk HEC for SIEM event forwarding
#
...
if [[ "$ENABLE_MONITORING" == "true" ]]; then
    ...
    if ! grep -q "^GRAFANA_ADMIN_PASSWORD=" "$INSTALL_DIR/.env" 2>/dev/null; then
        GRAFANA_PASS=$(openssl rand -base64 16 | tr -d '=+/')
        echo "" >> "$INSTALL_DIR/.env"
        echo "# Monitoring (generated by deploy.sh --monitoring)" >> "$INSTALL_DIR/.env"
        echo "GRAFANA_ADMIN_PASSWORD=${GRAFANA_PASS}" >> "$INSTALL_DIR/.env"
    ...
    # Configure Caddy reverse proxy for Grafana
    if ! grep -q "grafana" "$CADDYFILE" 2>/dev/null; then
        # Insert Grafana handler into Caddyfile before the closing brace of the Snapper block
        # We append a new block for /grafana/* sub-path proxying
        if [[ -n "$DOMAIN" ]]; then
            GRAFANA_BLOCK_LABEL="${DOMAIN}"
        else
            GRAFANA_BLOCK_LABEL=":${SNAPPER_PORT}"
        fi

        # Add a separate route block for Grafana
        cat >> "$CADDYFILE" <<CADDYEOF

# Grafana (managed by deploy.sh --monitoring)
${GRAFANA_BLOCK_LABEL} {
    handle_path /grafana/* {
...
CADDYEOF
        ...
        if caddy reload --config "$CADDYFILE" 2>/dev/null; then
            ok "Caddy configured to proxy /grafana/* → Grafana"
        else
            warn "Caddy reload failed — check $CADDYFILE"
        fi
    else
        ok "Caddy already configured for Grafana"
    fi

    log "Grafana available at: ${EXTERNAL_URL}/grafana/"
    log "  Username: admin"
    log "  Password: ${GRAFANA_PASS}"
fi

# ─── Step 10b: Splunk HEC (Optional) ─────────────────────────────────────
if [[ "$ENABLE_SPLUNK" == "true" ]]; then
    log "Configuring Splunk HEC integration..."

    if grep -q "^SIEM_SPLUNK_HEC_URL=" "$INSTALL_DIR/.env" 2>/dev/null; then
        warn "Splunk HEC already configured in .env"
    else
        read -r -p "Splunk HEC URL (e.g. https://splunk.example.com:8088/services/collector/event): " SPLUNK_URL
        read -r -p "Splunk HEC Token: " SPLUNK_TOKEN
        read -r -p "Splunk Index [main]: " SPLUNK_INDEX
        SPLUNK_INDEX="${SPLUNK_INDEX:-main}"

        # Test connectivity
        log "Testing Splunk HEC connectivity..."
        SPLUNK_TEST=$(curl -sk -o /dev/null -w "%{http_code}" \
            -X POST "$SPLUNK_URL" \
            -H "Authorization: Splunk $SPLUNK_TOKEN" \
            -d '{"event":"snapper_deploy_test"}' 2>/dev/null || echo "000")

        if [[ "$SPLUNK_TEST" == "200" ]]; then
            ok "Splunk HEC connection verified"
        else
            warn "Splunk HEC test returned HTTP $SPLUNK_TEST — check URL and token"
        fi

        # Write to .env
        cat >> "$INSTALL_DIR/.env" <<SPLUNKEOF

# Splunk HEC (generated by deploy.sh --splunk)
SIEM_OUTPUT=splunk
SIEM_SPLUNK_HEC_URL=${SPLUNK_URL}
SIEM_SPLUNK_HEC_TOKEN=${SPLUNK_TOKEN}
SIEM_SPLUNK_INDEX=${SPLUNK_INDEX}
SIEM_SPLUNK_SOURCETYPE=snapper:security
SIEM_SPLUNK_VERIFY_SSL=true
SPLUNKEOF

        ok "Splunk HEC configuration written to .env"

        # Restart app to pick up new env vars
        $COMPOSE_CMD up -d --force-recreate app
        ok "App restarted with Splunk HEC enabled"
    fi
fi
...
# SIEM output
if [[ -f "$INSTALL_DIR/.env" ]]; then
    SIEM=$(grep "^SIEM_OUTPUT=" "$INSTALL_DIR/.env" 2>/dev/null | cut -d= -f2 | tr '[:upper:]' '[:lower:]')
    if [[ -z "$SIEM" || "$SIEM" == "none" ]]; then
        sec_warn "SIEM output is not configured — audit events only stored locally"
        echo -e "         Action: Set SIEM_OUTPUT=splunk|syslog|webhook in .env"
    else
        sec_pass "SIEM output configured ($SIEM)"
    fi
fi

# Prometheus monitoring
if docker ps --filter "name=prometheus" --format '{{.Names}}' 2>/dev/null | grep -q prometheus; then
    sec_pass "Prometheus monitoring is running"
else
    sec_warn "Prometheus not running — no metrics collection"
    echo -e "         Action: Deploy with --monitoring flag to enable"
fi

# Grafana not exposed directly
GRAFANA_PORT_BINDING=$($COMPOSE_CMD port grafana 3000 2>/dev/null || echo "")
if [[ -z "$GRAFANA_PORT_BINDING" || "$GRAFANA_PORT_BINDING" == ":0" ]]; then
    # Not running or not exposed — either is fine
    :
elif echo "$GRAFANA_PORT_BINDING" | grep -q "^127\.0\.0\.1:"; then
    sec_pass "Grafana bound to 127.0.0.1 (proxied through Caddy)"
elif echo "$GRAFANA_PORT_BINDING" | grep -q "^0\.0\.0\.0:"; then
    sec_fail "Grafana exposed on 0.0.0.0 — should bind to 127.0.0.1 only"
    echo -e "         Fix: Use docker-compose.prod.yml (binds Grafana to 127.0.0.1)"
fi
