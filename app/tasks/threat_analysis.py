...
@celery_app.task(bind=True, max_retries=0, time_limit=10)
...
async def _analyze_signals_async():
    ...
    try:
        ...
        logger.debug(f"Failed to discover signal streams: {e}")
        return
...
async def _create_threat_event(
    ...
    try:
        ...
        logger.warning(f"Failed to create threat event: {e}")
