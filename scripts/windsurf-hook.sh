#!/bin/bash
# Snapper PreToolUse Hook for Windsurf (Codeium)
# This hook checks with Snapper before allowing any tool execution
#
# Installation:
# 1. Copy this script to ~/.codeium/windsurf/hooks/snapper_pre_tool_use.sh
# 2. Make it executable: chmod +x ~/.codeium/windsurf/hooks/snapper_pre_tool_use.sh
# 3. Merge into ~/.codeium/windsurf/hooks/hooks.json:
#    {
#      "pre_run_command": [
#        { "command": "~/.codeium/windsurf/hooks/snapper_pre_tool_use.sh" }
#      ],
#      "pre_write_code": [
#        { "command": "~/.codeium/windsurf/hooks/snapper_pre_tool_use.sh" }
#      ],
...
#    }
#
# Or run: snapper init --agent windsurf

# Source Snapper env if available
[ -f ~/.codeium/windsurf/.env.snapper ] && set -a && . ~/.codeium/windsurf/.env.snapper && set +a

SNAPPER_URL="${SNAPPER_URL:-https://76.13.127.76:8443}"
SNAPPER_AGENT_ID="${SNAPPER_AGENT_ID:-windsurf-$(hostname)}"
...
# Extract tool info - Windsurf format
...
case "$TOOL_NAME" in
    run_command|Bash|bash|shell|execute|computer)
    ...
    read_file|Read|read)
        ...
        FILE_OP="read"
        ;;
    write_code|Write|write|Edit|edit)
        REQUEST_TYPE="file_access"
        FILE_PATH=$(echo "$TOOL_INPUT" | jq -r '.file_path // .path // ""')
        FILE_OP="write"
        ;;
...
# Check response - fail closed with exit code 2 for Windsurf
if [ $? -ne 0 ]; then
...
case "$DECISION" in
    ...
    deny)
        echo "BLOCKED by Snapper - Rule: $RULE_NAME - Reason: $REASON" >&2
        exit 2
        ;;
        ...
        while true; do
            ...
            case "$STATUS" in
                approved)
                    ...
                    ;;
                denied)
                    echo "DENIED by approver" >&2
                    exit 2
                    ;;
                expired)
                    echo "Approval expired" >&2
                    exit 2
                    ;;
                pending)
