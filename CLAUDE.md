...
- **MCP Security Categories**: 13 security categories (`data_store`, `code_repository`, `filesystem`, `shell_exec`, `browser_automation`, `network_http`, `communication`, `cloud_infra`, `identity_auth`, `payment_finance`, `ai_model`, `monitoring`, `general`) in `app/data/category_rule_templates.py`. Classifier in `app/services/server_classifier.py` (tiers 1+2) and `app/services/bge_classifier.py` (tier 3). Auto-apply logic in `_auto_apply_category_rules()` in `app/routers/rules.py`. Category templates generate 3-5 rules per server based on security posture. `AUTO_CATEGORY_RULES=true` to enable.
- **MCP Catalog**: 27,129+ servers indexed from 5 registries (PulseMCP, Smithery, Glama, NPM, awesome-mcp-servers). Sync service in `app/services/mcp_catalog_sync.py`. API in `app/routers/mcp_catalog.py`. Servers are auto-classified into 13 security categories via a 3-tier classification engine (regex, keyword scoring, BGE embeddings).